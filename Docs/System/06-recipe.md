...
기계별 배정은 [08-machine-recipes.md](./08-machine-recipes.md).

# 아이템 레벨 규칙
# 일부 핵심 재료와 장비에는 레벨이 존재한다.
# 주괴 lv n은 용광로_n에서 같은 주괴 lv(n-1) 5개를 재련해 만든다. (08-machine-recipes.md)
...
철 각반 + 마력 코어x3 -> 마나강 각반
철 부츠 + 마력 코어x2 -> 마나강 부츠
# ↓ 속성 부여는 마법 부여대 레시피. 스크롤을 소모해 검/투구/흉갑/각반/부츠에 공용 적용 (08-machine-recipes.md)
>>>마나 포집
# 마나 포집기가 공기에서 정수를 만든다. 입력 없음. 정수가 마나 묶음이다 (유체 마나 없음).
(없음) -> 하급 마나 정수
# 함량 10. itemId `low_monster_mana_essence`
(없음) -> 중급 마나 정수
# 함량 50. itemId `mid_monster_mana_essence`
(없음) -> 상급 마나 정수
# 함량 100. itemId `high_monster_mana_essence`
(없음) -> 던전의 주인의 정수
# 함량 500. itemId `dungeon_master_essence`

>>>마법 스크롤
# 스크롤은 마법 관련 장비에 붙이는 재료. 속성·형태 각인은 마법 부여대 레시피.
...
# ↓ 로브 속성 부여도 마법 부여대 레시피 (08-machine-recipes.md)
