...
// 2키로 여는 3×3 구역 해금 UI. 구역 배치와 같은 모양의 버튼을 보여준다.
public class ZoneExpansionUI : MonoBehaviour
{
    ...
    private void RebuildZoneButtons()
    {
        ...
        for (int uiRow = 0; uiRow < ZoneManager.ZonesPerAxis; uiRow++)
        {
            int zoneY = ZoneManager.ZonesPerAxis - 1 - uiRow;
            for (int zoneX = 0; zoneX < ZoneManager.ZonesPerAxis; zoneX++)
            {
    ...
    private void CreateZoneButton(ZoneManager zoneManager, int zoneX, int zoneY)
    {
        ...
        bool canUnlock = !isCenter && !unlocked && zoneManager != null;
    ...
    private void EnsureUiHierarchy()
    {
        ...
        layout.constraintCount = ZoneManager.ZonesPerAxis;
    }
